...
def extract_keywords(note_content):
    prompt = f"Extract exactly 5 meaningful tags or keywords from the following note that accurately represent its content. Return a comma-separated list of these keywords after additional text or explanation:\n\n{note_content}"
    stream_handler = StreamHandler(st.empty())
    result = llm_chain({"question": prompt, "chat_history": []}, callbacks=[stream_handler])["answer"]
    
    # Extract all terms from the comma-separated list
                    ...
                    if user_journals:
                        for journal in user_journals:
                            with st.expander(f"{journal['name']}"):
                                ...
                                col1, col2, col3, col4 = st.columns([1, 1, 1, 7])  # Three columns for buttons, fourth for spacing
                                with col1:
